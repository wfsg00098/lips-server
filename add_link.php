...
<!-- Header -->
<header id="header">
    <a href="#" class="logo"><strong>内容添加(链接)</strong>&nbsp;&nbsp;(当前色号：<?php echo($brand_show."&nbsp;-&nbsp;".$series_show."&nbsp;-&nbsp;".$number); ?>)</a>
    <nav>
...
<section id="main">
    <div class="inner">
        <form method="post">
            ...
            链接内容<input type="text" name="content"><br><br>
            链接大小
            <select name="size">
                <?php
        ...
        if (isset($_POST["add"]))
        {
            ...
            else
            {
                if (substr($content,0,7) == "http://" or substr($content,0,8) == "https://")
                {
                    ...
                    if (!mysqli_num_rows($result))
                    {
                        $result = mysqli_query($sql,"insert `".$brand."_".$series."_".$number."` values('link','".$content."','".$describ."','".$size."','".$align."','".$bold."','".$italic."','','');");
                        if ($log_operation) logger($sql,$_COOKIE[$usercookie],"添加链接：".$brand." - ".$series." - ".$number." - ".$describ."(".$brand_show." - ".$series_show." - ".$number.")");
